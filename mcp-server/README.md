...
**1. Create `mcp-server/.env`** (gitignored):
...
**3. Connect Claude Code to localhost** — add to `~/.mcp.json`:
...
Handled automatically on every push to `main`:

1. Installs `mcp-server/` dependencies (`npm ci`)
2. Parses `.env.prod` and exports vars
3. `pm2 reload MCPServer --update-env` — zero-downtime restart
4. Falls back to `pm2 start` on first deploy (process doesn't exist yet)
5. Saves process list with `pm2 save`
