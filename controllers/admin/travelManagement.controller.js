...
//             t2.amount, t2.upi_ref, t2.status as paymentStatus, t3.res_status
//      FROM travel_db t1
//      LEFT JOIN transactions t2 ON t2.bookingid = t1.bookingId AND t2.category = :category
//      LEFT JOIN card_db t3 ON t1.cardno = t3.cardno
//      WHERE t1.date >= :startDate AND t1.date <= :endDate ${additionalWhereClause}
...
//     where: {
//       bookingid: bookingid,
//       status: [STATUS_WAITING, STATUS_CONFIRMED, STATUS_PAYMENT_PENDING]
//     }
//   });
...
function getAdditionalConditions(statuses, pickup, drop, replacementMap) {
  let additionalWhereClause = "";
  ...
  if (pickup && pickup.trim() !== "") {
    additionalWhereClause += " AND t1.pickup_point = :pickup";
    replacementMap.pickup = pickup;
  }

  if (drop && drop.trim() !== "") {
    additionalWhereClause += " AND t1.drop_point = :drop";
    replacementMap.drop = drop;
  }
...
export const fectchSummary = async (req, res) => {
  const { start_date, end_date, statuses, pickup, drop } = req.query;
  ...
  const additionalWhereClause = getAdditionalConditions(statuses, pickup, drop, replacementMap);
