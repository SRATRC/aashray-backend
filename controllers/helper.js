...
export async function checkSpecialAllowance(start_date, end_date, primary_booking, addons, cardno) {
    ...
    if (addons && addons.length > 0) {
        adhyayanRequests.push(... addons.filter((addon) => addon.booking_type === TYPE_ADHYAYAN));
    }

    if (adhyayanRequests.length > 0) {
        const shibirIds = adhyayanRequests.flatMap((req) => req.details ?. shibir_ids || []);
...
export async function checkRoomBookingProgress(start_date, end_date, primary_booking, addons) {
    ...
    if ((primary_booking && primary_booking.booking_type == TYPE_ROOM) || addon) {
        const startDate = new Date(start_date);
        const endDate = new Date(end_date);
        const checkinDate = new Date(primary_booking.details.checkin_date || addon.details.checkin_date);
        const checkoutDate = new Date(primary_booking.details.checkout_date || addon.details.checkout_date);
...
export function retrieveBookingIds(userBookingIdMap) {
    return Object.values(userBookingIdMap).map((bookingIdsByType) => Object.values(bookingIdsByType).flat()).flat();
}

export async function sendUnifiedEmailForBookedBy(userBookingIdMap, bookedBy, bookingStatus) {
    const flattenedMap = {};
    ...
    for (const [cardNo, bookingTypes] of Object.entries(userBookingIdMap)) {
        ...
        for (const [type, bookingIds] of Object.entries(bookingTypes)) { // Initialize array if not exists
    ...
    if (Object.keys(flattenedMap).length > 0) {
        await sendUnifiedEmail(isSelfBooking ? bookedBy.cardno : null, flattenedMap, bookedBy, bookingStatus);
    }
}
...
export function getWelcomeMessage(bookingStatus, country) {
    ...
    if (bookingStatus == BOOKING_STATUS_PENDING) {
        ...
        return(country && country != 'India') ? bookingCreate + ' NRIs can make payments for any bookings in pending status at the Research Center upon arrival.' : bookingCreate + " Payment is due within 24 hours to confirm any bookings in pending status.";
    }
...
export async function sendUnifiedEmail(cardno, bookingIds, bookedBy, bookingStatus = BOOKING_STATUS_CONFIRMED, template = 'unifiedBookingEmail') {
    ...
    if (cardno == null) {
        ...
        user = await CardDb.findOne({where: {
                cardno
            }});
    }
    ...
    if (wasUtsavBooked) {
        const includeOptions = [
            {
                ...
                where: {
                    id: Sequelize.col('UtsavBooking.utsavid')
                } // Changed from UtsavBookingDb.utsav_id
            }, {
                model: UtsavPackagesDb,
                attributes: ['name'],
                where: {
                    id: Sequelize.col('UtsavBooking.packageid')
                } // Changed from UtsavBookingDb.packageid
            }
        ];
        ...
        if (includeProfile) {
            includeOptions.push({
                ...
                attributes: ['issuedto'],
                where: {
                    cardno: Sequelize.col('UtsavBooking.cardno')
                }
            });
        }
        ...
        const utsavBookings = await UtsavBooking.findAll({
            ...
            where: {
                bookingId: {
                    [Sequelize.Op. in]: bookingIds[TYPE_UTSAV]
                }
            },
            order: [
                [
    ...
    if (wasAdhyanBooked) {
        ...
        includeOptions.push({
            ...
            where: {
                id: Sequelize.col('ShibirBookingDb.shibir_id')
            }
        });
        if (includeProfile) {
            includeOptions.push({
                ...
                attributes: ['issuedto'],
                where: {
                    cardno: Sequelize.col('ShibirBookingDb.cardno')
                }
            });
        }
        const adhyanBookings = await ShibirBookingDb.findAll({
            ...
            where: {
                bookingId: {
                    [Sequelize.Op. in]: bookingIds[TYPE_ADHYAYAN]
                }
            },
            order: [
                [
        ...
        adhyanBookings.forEach((adhyanBooking) => {
            adhyanBookingDetails.push({
                ...
                startdate: moment(adhyanBooking.dataValues.ShibirDb.start_date).format('Do MMMM, YYYY'),
                enddate: moment(adhyanBooking.dataValues.ShibirDb.end_date).format('Do MMMM, YYYY'),
    ...
    if (wasRajprvasBooked) {
        ...
        if (includeProfile) {
            includeOptions.push({
                ...
                attributes: ['issuedto'],
                where: {
                    cardno: Sequelize.col('TravelDb.cardno')
                }
            });
        }
        const travelBookings = await TravelDb.findAll({
            ...
            where: {
                bookingId: {
                    [Sequelize.Op. in]: bookingIds[TYPE_TRAVEL]
                }
            },
            order: [
                [
    ...
    if (wasRoomBooked) {
        ...
        if (includeProfile) {
            includeOptions.push({
                ...
                attributes: ['issuedto'],
                where: {
                    cardno: Sequelize.col('RoomBooking.cardno')
                }
            });
        }
        const roomBookings = await RoomBooking.findAll({
            ...
            where: {
                bookingid: {
                    [Sequelize.Op. in]: bookingIds[TYPE_ROOM]
                }
            },
            order: [
                [
    ...
    if (wasFlatBooked) {
        ...
        if (includeProfile) {
            includeOptions.push({
                ...
                attributes: ['issuedto'],
                where: {
                    cardno: Sequelize.col('FlatBooking.cardno')
                }
            });
        }
        const flatBookings = await FlatBooking.findAll({
            ...
            where: {
                bookingid: {
                    [Sequelize.Op. in]: bookingIds[TYPE_FLAT]
                }
            },
            order: [
                [
...
export async function createGuestsHelper(cardno, guests, t) {
    ...
    const newCardIds = unregisteredGuests.length > 0 ? await createCardIds(unregisteredGuests.length) : [];
    ...
    if (guestsToCreate.length > 0) {
        await GuestRelationship.bulkCreate(guestsToCreate.map((guest) => ({cardno: cardno, guest: guest.cardno, type: guest.guest_type, updatedBy: cardno})), {transaction: t});
    }
...
export async function createCardIds(count) { // Convert array to Set for O(1) lookups if needed
    const existingIds = await CardDb.findAll({attributes: ['cardno'], raw: true}).then((cards) => cards.map((card) => card.cardno));
    ...
    if (usedIds.size > RANDOM_THRESHOLD) { // With many existing IDs, use sequential generation with validation
    ...
    } else { // With fewer existing IDs, random generation might be more efficient
        let attempts = 0;
      ...
      if (attempts> = MAX_ATTEMPTS && newIds.length < count) {
            console.warn(`Random generation inefficient, switching to sequential for remaining ${
                count - newIds.length
            } IDs`);
